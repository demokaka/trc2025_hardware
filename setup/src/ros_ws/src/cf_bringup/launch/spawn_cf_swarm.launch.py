...
#  6) Start gazebo_tf_publisher to publish world->robot TFs     => not necessary for crazyswarm
#  7) Start RViz2 with a given config                           => not necessary for crazyswarm
#  8) Start cf2 SITL processes for each agent
#  9) Start a crazyflie server to handle commands               => implemnted in crazyswarm_init.launch.py
...
def _prepare(context, *args, **kwargs):
    ...
    # rviz_config_file   = LaunchConfiguration("rviz_config_file").perform(context)  # not necessary for crazyswarm
    ...
    for a in agents:
        ...
        cffirm_udp = int(a.get("cffirm_udp_port", 19950 + cf_id-1))
        cflib_udp  = int(a.get("cflib_udp_port",  19850 + cf_id-1))
